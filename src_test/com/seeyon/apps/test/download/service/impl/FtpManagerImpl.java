...
import com.seeyon.apps.test.download.dao.QuartzDao;
import com.seeyon.apps.test.download.service.FtpManager;
import com.seeyon.apps.test.util.FtpUtil;
import com.seeyon.apps.test.util.JDBCUtil;
...
public class FtpManagerImpl implements FtpManager {
    ...
    //日志
    private static final Log log = CtpLogFactory.getLog(FtpManagerImpl.class);
    ...
    public void official() {
        ...
        if (resultList.size() != 0) {
            ...
            //2.上传至ftp服务器文件夹
            String ip = AppContext.getSystemProperty("test.ftpIP");
            int port = 21;
            String userName = AppContext.getSystemProperty("test.ftpAccount");
            String passWord = AppContext.getSystemProperty("test.ftpPassWord");
            ...
            for (int i = 0; i < resultList.size(); i++) {
                ...
                Long sender = 402616456459864064L;//发送者ID
                MessageReceiver receiver = new MessageReceiver(null, 402616456459864064L, "message.link.url", "");//接收者
                try {
